...
def _iterate_sparse_X(X):
    ...
    ------
    row : torch.Tensor
        A densified row of the sparse matrix.
...
def _split_node(node, threshold, branching_factor):
    ...
    branching_factor: int
        Maximum number of BFSubclusters in each node
...
class _BFNode:
    ...
    init_centroids_ : torch.Tensor of shape (branching_factor + 1, n_features)
        Manipulate ``init_centroids_`` throughout rather than centroids_ since
        the centroids are just a view of the ``init_centroids_`` .

    centroids_ : torch.tensor of shape (branching_factor + 1, n_features)
        View of ``init_centroids_``.

    """

    def __init__(self, *, threshold, branching_factor, is_leaf, n_features, dtype, device):
        ...
        self.subclusters_ = []
        self.init_centroids_ = torch.zeros((branching_factor + 1, n_features), dtype=dtype, device=device)
    ...
    def insert_bf_subcluster(self, subcluster, set_bits):
        ...
        a = torch.matmul(self.centroids_, subcluster.centroid_)
        sim_matrix = a / (torch.sum(self.centroids_, axis=1) + set_bits - a)
        closest_index = torch.argmax(sim_matrix)
        ...
        if closest_subcluster.child_ is not None:
            ...
            # We need to redistribute the subclusters in
...
class _BFSubcluster:
    ...
    device: torch.device
        used to ensure newly created tensors are on same device as workflow
...
class BitBirch:
    ...
    subcluster_centers_ : torch.Tensor
    ...
    subcluster are updated. This is done recursively until the properties of
    ...
    def _fit(self, X):
        ...
        centroids = torch.cat([leaf.centroids_ for leaf in self._get_leaves()])
    ...
    def get_centroids(self):
        ...
        for leaf in self._get_leaves():
            for subcluster in leaf.subclusters_:
                centroids.append(subcluster.centroid_.cpu().numpy())
