import numpy as np
import pandas as pd
from iSIM.sampling import medoid_sampling, outlier_sampling
from iSIM.utils import binary_fps
from rdkit.Chem.Scaffolds import MurckoScaffold

from bitbirch.cluster_control import birch_analysis, chi, dbi, dunn, jt_isim, radius


class ClusterAnalysis:
    def __init__(self, brc, fingerprints, smiles, double=False):
        self.brc = brc
    ...
    def forward(self):
        ...
        (
            self.n_biggest_cluster,
            self.isim_biggest_cluster,
            self.isim_outliers,
            self.isim_medoids,
            self.radius_value,
        ) = self.biggest_cluster_metrics(biggest_cluster, fps_biggest_cluster)
...
def check_cluster_structure(brc):
    ...
    for index, node in enumerate(brc._get_leaves()):
        for c in node.subclusters_:
            ...
            assert c.n_samples_ == len(
                mol_j
            ), f"Fatal! N_j:{c.n_samples_} and len(mol_j):{len(mol_j)} are incompatible."
