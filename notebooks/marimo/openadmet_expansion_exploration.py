...
def _():
    ...
    from useful_rdkit_utils import compare_datasets
...
def _(alt, mo, unique_cluster_df):
    chart = mo.ui.altair_chart(
        alt.Chart(unique_cluster_df[["PC1", "PC2", "image"]])
        .mark_circle(size=60)
        .encode(
            x=alt.X(
                "PC1",
    ...
    def calc_sali(df, assay):
        ...
        for k, v in tqdm(train_df.groupby("cluster"), desc=assay, disable=hide_progress):
            ...
            for (_, row_1), (_, row_2) in combinations(v.iterrows(), 2):
                ...
                res_list.append(
                    [row_1.SMILES, row_1.Name, row_1[assay], row_2.SMILES, row_2.Name, row_2[assay], sim, sali]
                )
        sali_df = pd.DataFrame(
            res_list, columns=["SMILES_1", "Name_1", "Val_1", "SMILES_2", "Name_2", "Val_2", "Tanimoto", "SALI"]
        )
...
def _(Chem, np, sali_df):
    num_to_show = 12
    mol_list = [
        Chem.MolFromSmiles(x) for x in np.array(list(zip(sali_df.SMILES_1, sali_df.SMILES_2))).flatten()[:num_to_show]
    ]
